...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={The Invariance of Dithering Efficiency Under Non-Conformal Projections},
...
\author{
  ...
  \texttt{remi.millerat--gallot@universite-paris-saclay.fr} \\
  \textit{Master's student at UPSaclay}
}
\date{January 2026}
...
\section{Introduction}
    ...
    This paper provides the formal mathematical proof for why such complexity is unnecessary. We utilize Anisotropic Jacobian-Weighted Dithering (AJWD) as a theoretical baseline to investigate the behavior of noise under non-conformal mappings. Our analysis demonstrates that Spatially Uniform PDF Dithering remains the optimal strategy because the projection operator transforms both the dithering noise and the quantization error identically. Consequently, the local Masking Ratio remains invariant. We define the ``Anisotropy Fallacy''---the mistaken belief that isotropic noise is desirable on the sphere surface---and show that standard uniform dithering naturally satisfies the requirement for covariant masking, rendering explicit corrections computationally wasteful.
...
\subsection{Unit Vector Encodings}
    ...
    To address these inherent geometric inefficiencies, Meyer et al.~\cite{meyer2010} introduced Octahedral Normal Vectors (ONV) in 2010. By projecting the sphere onto an inscribed octahedron using the $L_1$ norm and unwrapping it to a square, they eliminated the need for expensive transcendental functions while ensuring a nearly uniform distribution of precision. As of 2026, Octahedral encoding has superseded older methods to become the industry gold standard.
...
\subsection{Masking Ratio Invariance and the Anisotropy Fallacy}
    ...
    The efficacy of dithering is determined by the local \textit{Masking Ratio} $\gamma(\mathbf{u})$, defined here as the ratio between the magnitude of the projected noise and the magnitude of the projected quantization step. Substituting the linear approximations, we observe that the Jacobian operator scales both terms:
    
    \begin{equation}
        \gamma(\mathbf{u}) = \frac{\| \bm{\eta}_{S^2} \|_2}{\| \mathbf{e}_{S^2} \|_2} \approx \frac{\| \mathbf{J}_{\Phi}(\mathbf{u}) \cdot \bm{\eta}_{\Omega} \|_2}{\| \mathbf{J}_{\Phi}(\mathbf{u}) \cdot \mathbf{e}_{\Omega} \|_2}
    \end{equation}
    ...
    \begin{itemize}
        \item \textbf{Spatially Uniform Dithering (Optimal):} The uniform noise $\bm{\eta}_{\Omega}$ is naturally stretched by $\mathbf{J}_{\Phi}$ into the same anisotropic shape as the error. The noise ``streak'' perfectly overlays the quantization ``streak'', ensuring consistent coverage.
        \item \textbf{Isotropic Correction (Sub-optimal):} By attempting to force the projected noise $\bm{\eta}_{S^2}$ to be circular, this strategy introduces a geometric mismatch. If energy is preserved, the noise is insufficiently long to cover the banding (under-dithering). If the amplitude is boosted to cover the banding, the noise becomes excessively wide perpendicular to the band, wasting variance.
    \end{itemize}
...
\subsection{Implementation}
    ...
    The WebGPU simulation framework and full source code are available at: \url{https://github.com/Bafy78/dithering-in-octahedral}
    ...
    For every combination of encoding (Cartesian vs. Hemi-Oct), dithering strategy (Uniform vs. AJWD), and bit-depth, the system renders and exports a $256 \times 256$ image of the highlight. These samples are then compared against a ``Ground Truth'' render (unquantized 32-bit float normals) using an offline Python script to compute the FSIM indices presented in Section 4.2.
...
\subsection{Results and Evaluation}
    ...
    To provide an objective metric for these perceptual observations, we computed the Feature Similarity Index (FSIM) for each sample against the 32-bit ground truth.
    ...
    \begin{table}[ht]
        ...
        \textbf{Uniform (TPDF / Recommended)} & \textbf{0.9176} & \textbf{0.8932} & \textbf{0.9314} \\
    ...
    It is worth noting that at the Pole (Table \ref{tab:fsim_scores}, Column 1), the scores for Uniform TPDF and AJWD converge identically ($0.9176$). This serves as a validation of our implementation: in regions of minimal distortion, the Jacobian approaches the identity matrix, and the weighted strategy naturally collapses into standard uniform dithering. The divergence in scores is strictly confined to regions where the anisotropic projection is active.

    While the visual results focus on the 8-bit worst-case scenario to maximize artifact visibility, the relative performance remains invariant at higher precisions. At 10-bit, Uniform TPDF maintains its lead in the Diagonal region (FSIM 0.981 vs AJWD 0.979), confirming that the geometric efficiency of uniform dithering is independent of the quantization step size $\Delta$. At higher bit-depths, while the absolute magnitude of artifacts diminishes, the Efficiency Ratio $R(\mathbf{u})$ remains invariant.

    \subsubsection{Computational Cost and Performance}

    In addition to visual quality, we evaluated the execution cost of the proposed strategies. While standard uniform dithering requires only a single scalar addition, the AJWD strategy proves prohibitively expensive for real-time applications. 
    
    Our implementation utilized analytic partial derivatives of the projection operator rather than screen-space hardware gradients. A static analysis of the generated shader assembly reveals that the Jacobian weighting logic adds approximately \textbf{54 ALU instructions}, including expensive transcendental operations (square roots and reciprocal divisions).
    
    Given that the standard Hemi-Octahedral encoding requires fewer than 20 instructions, AJWD increases the total cost of the quantization pass by roughly \textbf{300\%}. Since this threefold increase in cost yields strictly lower FSIM scores, AJWD represents a dominated strategy in terms of cost-benefit.

\section{Discussion}
    
    \subsection{The Intuition Trap: Integration vs. Masking}
    
    The hypothesis that dithering requires Jacobian weighting stems from conflating \textit{energy preservation} with \textit{artifact masking}. In contexts like Monte Carlo integration, weighting by $\mathbf{J}(\mathbf{u})^{-1}$ is strictly necessary to preserve signal energy against domain distortion. 
    
    However, dithering is a local contrast problem, not an integration problem. We seek to maintain the ratio between noise amplitude and quantization step size, not the total noise energy over the sphere. As proven in Eq. (4), the projection operator $\Phi$ applies the same linear transformation to both the error and the noise. Consequently, inverse-weighting creates a category error: it attempts to force isotropic noise onto an anisotropic error, breaking the covariance required for effective masking.

    \subsection{Temporal Stability and Supersampling}
    
    Modern pipelines rely on Temporal Anti-Aliasing (TAA) to accumulate dithered samples and mitigate quantization artifacts. Cigolle et al.~\cite{cigolle2014} explicitly identify the open challenge of adding noise that remains ``temporally coherent under animation.'' We posit that AJWD exacerbates this instability by coupling noise amplitude to the local Jacobian $J(\mathbf{u})$. As an object rotates, this coupling induces time-variant noise statistics that complicate the neighborhood clamping heuristics fundamental to TAA~\cite{karis2014}. In contrast, uniform dithering provides a consistent variance baseline, minimizing ghosting and ensuring stable history rectification.
    
\section{Conclusion}

    We have presented a mathematical proof and experimental validation for the invariance of dithering efficiency under Hemi-Octahedral projection. By contrasting standard methods against a Jacobian-corrected theoretical model, we demonstrated that the projection operator preserves the local Masking Ratio by deforming both signal and noise identically.
    
    This finding resolves the apparent paradox of why uniform dithering works well on distorted maps: the geometric distortion is not a hindrance to masking, but a covariant feature. We conclude that the standard approach---16-bit Hemi-Octahedral with Uniform noise---is not just an engineering compromise, but the Pareto-optimal solution for G-Buffers.
